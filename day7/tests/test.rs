...
fn get_amplifier_result(mode: AmplificationCircuitMode, program: &str, min: i32, max: i32) -> i32 {
    let program: Vec<i32> = program
        .split(',')
        .map(|x| x.parse().expect("Invalid number"))
        .collect();

    AmplificationCircuit::get_largest_signal(mode, program, min, max)
}

#[test]
fn test1() {
    let program = "3,15,3,16,1002,16,10,16,1,16,15,15,4,15,99,0,0";
    assert_eq!(get_amplifier_result(AmplificationCircuitMode::Serial, program, 0, 4), 43210);
}

#[test]
fn test2() {
    let program = "3,23,3,24,1002,24,10,24,1002,23,-1,23,101,5,23,23,1,24,23,23,4,23,99,0,0";
    assert_eq!(get_amplifier_result(AmplificationCircuitMode::Serial, program, 0, 4), 54321);
}

#[test]
fn test3() {
    let program = "3,31,3,32,1002,32,10,32,1001,31,-2,31,1007,31,0,33,1002,33,7,33,1,33,31,31,1,32,31,31,4,31,99,0,0,0";
    assert_eq!(get_amplifier_result(AmplificationCircuitMode::Serial, program, 0, 4), 65210);
}

#[test]
fn test4() {
    let program = "3,26,1001,26,-4,26,3,27,1002,27,2,27,1,27,26,27,4,27,1001,28,-1,28,1005,28,6,99,0,0,5";
    assert_eq!(get_amplifier_result(AmplificationCircuitMode::Feedback, program, 5, 9), 139629729);
}
...
fn test5() {
    ...
    assert_eq!(get_amplifier_result(AmplificationCircuitMode::Feedback, program, 5, 9), 18216);
}
