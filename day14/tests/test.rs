...
fn generate_cookbook(input: &str) -> CookBook {
    let mut cookbook = CookBook::new();
    
    for line in input.split('\n') {
        let recipe = Recipe::parse(&line);
...
fn test1() {
    ...
    let cookbook = generate_cookbook(&input);
